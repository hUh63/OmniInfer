...
`serve --startup-timeout <seconds>` applies to both gateway readiness and backend/model cold-start readiness and defaults to 300 seconds. On WSL2 ROCm, a readiness timeout during the first 120 seconds triggers targeted runtime cleanup, a 30-second cooldown, and one retry within the same total budget so a cold Triton cache can recover without a second user command. OmniInfer does not terminate the WSL distribution or unrelated workloads. Early process exits and explicit total budgets below 270 seconds are not retried.
