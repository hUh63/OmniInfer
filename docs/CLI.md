...
On unified-memory WSL2 ROCm systems, the managed launcher limits the default vLLM KV cache to 20% of WSL memory, capped at 4 GiB, so an APU's large shared-memory report cannot trigger a host-memory overcommit. Advanced users can override this policy with vLLM's `--kv-cache-memory-bytes` or `--gpu-memory-utilization` backend argument.
