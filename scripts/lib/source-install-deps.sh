...
No sudo option on Ubuntu hosts with NVIDIA apt repo: bash scripts/platforms/linux/setup-cuda-cublas-local.sh, then rerun with CUDAToolkit_ROOT=$HOME/.local/cuda-12.5.
EOF
}
